...
class MyEventController extends Controller
{
    ...
    {   
		...
		$order_details = Order::select(DB::raw('SQL_CALC_FOUND_ROWS *, orders.id, orders.first_name, orders.last_name, orders.email, ticket_groups.name, order_details.quantity, orders.created_at, ticket_groups.price'))
    ...
    {   
        ...
        $order_details = Order::select(DB::raw('orders.id, orders.first_name, orders.last_name, orders.email, ticket_groups.name, order_details.quantity, orders.created_at, ticket_groups.price'))
                ->join('order_details', 'orders.id', '=', 'order_details.order_id')
                ->join('ticket_groups', 'ticket_groups.id', '=', 'order_details.ticket_group_id')
                ->where('orders.event_id', $event->id)
                ->where('orders.order_status', 2)
